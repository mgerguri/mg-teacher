...
import { checkForUpdate, getDownloadUrlForCurrentOs, getReleaseNotesUrl, UpdateCheckResult } from '../lib/update-check'
...
export default function UpdateBanner() {
  ...
  async function viewWhatsNew() {
    const { open } = await import('@tauri-apps/api/shell')
    await open(getReleaseNotesUrl(update!.latestVersion))
  }
  ...
  return (
    <div className="bg-blue-600 text-white text-sm px-4 py-2 flex items-center justify-center gap-3">
      ...
      <button onClick={viewWhatsNew} className="underline hover:no-underline">
        {t('update.whatsNew')}
      </button>
